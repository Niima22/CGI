...
public class ShiftSwapRequest {
    ...
    public void updateStatus(String status) {
        if (this.status.equals("APPROVED") && !status.equals("APPROVED")) {
            throw new IllegalArgumentException("Approved shift swap requests cannot be changed.");
        }
        if (!status.equals("APPROVED") && !status.equals("REJECTED") && !status.equals("CANCELLED")) {
            throw new IllegalArgumentException("Unsupported shift swap request status: " + status);
        }
        this.status = status;
    }
