...
public class PlanningValidationServiceImpl implements PlanningValidationService {
    ...
    public List<PlanningProblemResponse> validate(
        ...
        validateWeeklyWork(weekStart, agents, assignments, problems);
    ...
    private void validateWeeklyWork(
        ...
        for (PlanningAgent agent : agents) {
            ...
            long offDays = 7 - workedDays;
            if (hours != TARGET_WEEKLY_HOURS) {
                addError(problems, "AGENT_NOT_40H",
                        agent.getFullName() + " is scheduled for " + hours + "h instead of exactly 40h.",
                        agent.getId(), null);
            }
            if (workedDays != TARGET_WORKING_DAYS || offDays != 2) {
                addError(problems, "INVALID_OFF_DAYS",
                        agent.getFullName() + " must work exactly 5 days and have exactly 2 OFF days; found "
                                + workedDays + " working days and " + offDays + " OFF days.",
                        agent.getId(), null);
            }
        }
    }
