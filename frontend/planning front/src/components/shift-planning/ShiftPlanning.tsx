...
export default function ShiftPlanning() {
  ...
  const unplanned = allOrders.filter((o) => o.status === "Open" || o.status === "In progress").length;
      ...
      <div className="relative mx-auto max-w-[1180px]">
        <div className="flex overflow-hidden rounded-[22px] bg-white shadow-[0_30px_70px_-30px_rgba(20,50,90,0.35)] ring-1 ring-black/[0.03]">
          ...
          <aside className="hidden w-[60px] flex-col items-center gap-1 border-r border-[var(--sp-line)] py-6 sm:flex">
            {([CalendarDays, Plus, CheckCircle2, Printer, PenLine, BarChart3, CalIcon] as const).map(
              (Icon, i) => (
                <SidebarIcon
                  key={i}
                  icon={Icon}
          ...
          <div className="flex-1">
            ...
            <div className="flex">
              ...
              <div className="hidden w-[210px] shrink-0 border-r border-[var(--sp-line)] p-5 lg:block">
                ...
                <Legend color="var(--sp-green)" label="Planned" value={planned} active={false} onClick={() => {}} />
                <Legend color="var(--sp-cyan)" label="Unplanned" value={unplanned} active={false} onClick={() => {}} />
                <Legend color="var(--sp-purple)" label="Predictive" value={predictive} active={false} onClick={() => {}} />
              </div>
                    ...
                    <div className="flex rounded-lg border border-[var(--sp-line)] p-0.5 text-xs">
                      {(["Day", "Week"] as const).map((v) => (
                        <button
                          ...
                          style={{
                            background:
                              view === v ? "color-mix(in oklch, var(--sp-page) 70%, white)" : "transparent",
