...
function PlanningPage() {
  ...
  async function loadAgents() {
    try {
      ...
      setError("Impossible de charger les agents du planning.");
    }
  }
  ...
  async function loadPlanning() {
    ...
    try {
      ...
      setError("Impossible de charger le planning de cette semaine.");
    }
  }
  ...
  async function setUnavailability(agent: PlanningAgent, date: string, reason: string | null) {
    try {
      const response = await authenticatedFetch(`/api/plannings/week/${weekStart}/unavailability`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ agentId: agent.id, date, reason }),
      });
      if (!response.ok) throw new Error(await planningApiError(response));
      const data = (await response.json()) as WeeklyPlanning;
      setPlanning(data);
  ...
  function updateCell(agent: PlanningAgent, date: string, value: string) {
    ...
    if (value === "CONGE" || value === "ABSENT" || value === "TELETRAVAIL") {
                ...
                <tr key={agent.id} className="group border-b border-slate-100 last:border-b-0">
                  ...
                  {days.map((day, index) => {
                    ...
                    return (
                      <td
                        key={day}
                        className={`border-r border-slate-100 p-0.5 align-top ${
...
function ShiftCell({
  ...
  if (readOnly || isLocked) {
    return (
      <div
        className={`relative min-h-8 rounded-md border px-1 py-0.5 text-[9px] transition ${visual.card}`}
      >
        <div className="flex items-center gap-2">
          <span className={`h-2 w-2 rounded-full ${visual.dot}`} />
          <div className="font-semibold">{mainLabel}</div>
        </div>
        {isTelework && <TeleworkBadge />}
        {isLocked && (
          <button
            type="button"
            className="mt-1 flex items-center gap-1 text-[9px] uppercase text-amber-700"
            title={frozenLabel}
  ...
  return (
    <div className="relative">
      <Select value={value} onValueChange={onChange}>
        <SelectTrigger
          className={`h-9 w-full rounded-md border px-1 shadow-none transition hover:border-slate-400 ${visual.card}`}
        >
          <div className="min-w-0 text-left">
            <div className="flex items-center gap-2">
              <span className={`h-2 w-2 rounded-full ${visual.dot}`} />
              <span className="truncate text-[10px] font-semibold">{mainLabel}</span>
            </div>
            {isTelework && <TeleworkBadge />}
        ...
        <SelectContent>
          ...
          <SelectItem value="TELETRAVAIL">Teletravail</SelectItem>
...
function TeleworkBadge() {
  return (
    <span className="mt-0.5 inline-flex w-fit rounded-full border border-sky-200 bg-white/85 px-1.5 py-0 text-[8px] font-semibold leading-3 text-sky-700">
      Teletravail
    </span>
  );
}
